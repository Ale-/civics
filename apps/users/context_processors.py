...
def current_user_hello(request):
    ...
    if current_user.is_staff:
        current_user_hello = _("Hola <a class='username' title='Ve a tu perfil de usuari@' href='%(link)s'>%(name)s <span class='icon-agent'></span></a>") % { 'link' : reverse('users:dashboard_staff') , 'name' : request.user.username }
